# %% [markdown]
# # Observatoire PME Maroc — Analyse Exploratoire des Données (Jalon 1)
...
# Ce notebook présente l'analyse exploratoire complète des données de cybermenaces
# collectées lors du Jalon 1, à partir de deux sources gratuites :
...
# Ajout du répertoire racine du projet pour importer la couche d'abstraction storage
...
# %%
# Aperçu des lignes issues de chaque source
...
# %% [markdown]
# ## 3. Répartition par catégorie de menace (Taxonomie AUSIM/CMRPI)
...
# %% [markdown]
# *Note explicative : La prédominance des catégories `ransomware_malware` et
# `ddos_extortion` reflète directement les spécificités des flux collectés
# (les URLs URLhaus diffusent principalement des charges utiles malveillantes,
# tandis que les adresses AbuseIPDB proviennent de listes de blocage IP liées
# au brute-force et aux botnets), et ne constitue aucunement une anomalie de
# traitement.*

# %% [markdown]
# ### 3bis. Croisement Source × Catégorie
...
# %% [markdown]
# ## 4. Qualité des données (Validation de complétude)
...
# %% [markdown]
# **Lecture** : les 9 champs du schéma `ThreatEvent` (validés par Pydantic) sont
# à 100% de complétude par construction — toute ligne avec un champ requis
# manquant est rejetée avant stockage (`processing/validate.py`). Les champs
# d'enrichissement `country_code` et `sector_hint` sont volontairement moins
# renseignés à ce stade (limites connues, documentées dans le README).
...
# %% [markdown]
# ## 6. Analyse de sévérité (AbuseIPDB uniquement)

# %%
abuse_df = df[df["source"] == "abuseipdb"]
...
# %% [markdown]
# ## 8. Statut des indicateurs (online / offline / reported)
...
# %% [markdown]
# ## 9. Vérification de la déduplication
# Contrôle rapide : la clé canonique `(indicator_value, source, date_added)`
# ne doit produire aucun doublon dans les données stockées.

# %%
dedup_check = df.duplicated(subset=["indicator_value", "source", "date_added"]).sum()
...
# %% [markdown]
# ## 10. Synthèse des livrables du Jalon 1
#
# Ce notebook conclut la validation du **Jalon 1** de l'Observatoire des
# Menaces Cyber pour PME Marocaines.
#
# **Ce qui a été livré :**
# - Deux collecteurs fonctionnels : `collectors/urlhaus.py` (source primaire,
#   CSV, sans authentification) et `collectors/abuseipdb.py` (source
#   secondaire, API JSON avec clé).
# - Un pipeline complet fetch → validate → categorize → save, reposant sur
#   `processing/validate.py` (schéma Pydantic `ThreatEvent`) et
#   `processing/taxonomy.py` (catégorisation AUSIM/CMRPI par mots-clés).
# - Une couche de stockage abstraite (`storage/repository.py`) garantissant
#   la déduplication sur la clé `(indicator_value, source, date_added)`.
# - Une suite de tests unitaires (9/9 passés) couvrant le stockage, la
#   validation et la taxonomie.
#
# **Chiffres clés :**
